...
export const getPosts = (page?: string, tag?: string, search?: string): Promise<PagedResponse<Array<Post>>> =>
  fetch(ENDPOINTS.posts.list(page, tag, search)).then<PagedResponse<Array<Post>>>(json);

export const getPost = (id: number): Promise<Post> =>
  fetch(ENDPOINTS.posts.get(id)).then<Post>(json);

export const getPostsByFeed = (feedId: number, page?: string, search?: string): Promise<PagedResponse<Array<Post>>> =>
  fetch(ENDPOINTS.posts.getByFeed(feedId, page, search)).then<PagedResponse<Array<Post>>>(json);
